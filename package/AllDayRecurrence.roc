...
AllDayRecurrence(id) :: {
	...
	collect = |initial, budget| {
		...
		while occurrences.len() < budget.max_occurrences {
			...
			match batch.status {
				End => return Ok({ occurrences, date_steps, zone_segments, status: Complete })
				Limited(progress) => return Ok({ occurrences, date_steps, zone_segments, status: Limited(progress) })
				Item(item) => {
		...
		Ok({ occurrences, date_steps, zone_segments, status: Limited({ cursor: state, reason: OutputLimit }) })
	}
	Next(id) : {
		date_steps : U64,
		date_buffered : U64,
	...
	next = |initial, limits| {
		...
		match state.pending {
			None => {
				...
				match batch.status {
					End => return Ok({ date_steps, date_buffered: batch.buffered, zone_segments: 0, zone_buffered: 0, status: End })
					Limited(progress) => {
						...
						return Ok({ date_steps, date_buffered: batch.buffered, zone_segments: 0, zone_buffered: 0, status: Limited({ cursor: { ..state, dates: progress.cursor }, reason }) })
					}
					Item(item) => {
						date = match CalendarDate.from_fields(Gregorian, GregorianDate.to_fields(item.date)) {
							Ok(value) => value
							Err(_) => crash "validated Gregorian date"
						}
		...
		Ok({ date_steps, date_buffered: state.date_buffered, zone_segments: zone.segments, zone_buffered: zone.buffered, status })
	}
