...
def verify_interchange(target: str) -> None:
    ...
    for mode in ("dev", "speed"):
        ...
        for date, clock, english_date in (("0000-01-01", "00:00:00", "1 Jan 0000"),
                            ("2026-09-07", "09:30:00.0012", "7 Sep 2026"),
                            ("+2147483647-12-31", "23:59:59.999999", "31 Dec +2147483647")):
        ...
        for scope in range(1, 10):
            failed_civil = subprocess.run(command + [str(scope), english_date], capture_output=True, timeout=5)
            marker = scope + (4 if scope <= 6 else 5)
            start = failed_civil.stderr.find(f"mark={marker} ".encode())
            end = failed_civil.stderr.find(f"mark={marker + 1} ".encode())
...
def verify_date_export(target: str) -> None:
    """Export finite definitions, never their logical occurrence domain."""
    roc = os.environ.get("ROC", "roc")
    source = "tests/date_export_resource/main.roc"
    subprocess.run([roc, "check", source], cwd=ROOT, check=True, timeout=120)
    for mode in ("dev", "speed"):
        binary = BUILD / f"date-export-{mode}"
        subprocess.run([roc, "build", source, f"--opt={mode}", f"--target={target}",
                        f"--output={binary}", "--no-cache"], cwd=ROOT, check=True, timeout=120)
        for ownership in ("owned", "shared", "sliced"):
            for count in (0, 1, 64, 4096):
                observations = []
                for horizon in (2001, 2000000000):
                    ...
                    if result.returncode or result.stdout != f"FREQ=DAILY;INTERVAL=1{months};WKST=MO".encode():
                        raise RuntimeError(f"{mode}/{count}/{ownership}/{horizon}: DATE export failed: {result.stderr!r}")
                    match = re.search(rb" work=((?:\d+,){6}\d+)\n$", result.stderr)
                    if match is None:
                        raise RuntimeError("missing DATE export allocation observations")
                    traffic = tuple(int(value) for value in match[1].split(b","))
                    if traffic[2] != 0 or traffic[3] > ceiling or any(value > 4096 for value in traffic[4:]):
                        raise RuntimeError(f"DATE definition/export/consumption bounds exceeded: {traffic}")
                    observations.append(traffic)
                if observations[0] != observations[1]:
                    raise RuntimeError(f"DATE export resource use depends on query horizon: {observations}")
                print(f"PASS DATE export {mode}/{count}/{ownership}: input/construct/access/export/cursor/first/resume requested bytes {observations[0]}")
        failed = subprocess.run([binary, "64", "shared", "2000000000", "0"], capture_output=True, timeout=5)
        if failed.returncode == 0 or b"ROC_ASSERT_FAILED" not in failed.stderr:
            raise RuntimeError(f"{mode}: DATE export allocation failure control failed")
...
def verify_schedule_definition(target: str) -> None:
    """Prepared declarations retain native labels and avoid repeated sorting."""
    roc = os.environ.get("ROC", "roc")
    source = "tests/schedule_definition_resource/main.roc"
    subprocess.run([roc, "check", source], cwd=ROOT, check=True, timeout=120)
    for mode in ("dev", "speed"):
        binary = BUILD / f"schedule-definition-{mode}"
        subprocess.run([roc, "build", source, f"--opt={mode}", f"--target={target}",
                        f"--output={binary}", "--no-cache"], cwd=ROOT, check=True, timeout=120)
        for ownership in ("owned", "shared", "sliced"):
            for count in (0, 1, 64, 4096):
                observations = []
                for horizon in (2001, 200000):
                    ceiling = 65536 + count * 1024
                    result = subprocess.run([binary, str(count), ownership, str(horizon), str(ceiling)], capture_output=True, timeout=5)
                    if result.returncode or result.stdout != b"schedule-definition":
                        raise RuntimeError(f"{mode}/{count}/{ownership}/{horizon}: definition failed: {result.stderr!r}")
                    match = re.search(rb" work=((?:\d+,){5}\d+)\n$", result.stderr)
                    if match is None:
                        raise RuntimeError("missing schedule definition allocation observations")
                    traffic = tuple(int(value) for value in match[1].split(b","))
                    if traffic[1] > ceiling or traffic[2] != 0 or any(value > 16384 for value in traffic[3:]):
                        raise RuntimeError(f"schedule definition resource bounds exceeded: {traffic}")
                    observations.append(traffic)
                if observations[0] != observations[1]:
                    raise RuntimeError(f"definition work depends on query horizon: {observations}")
                print(f"PASS schedule definition {mode}/{count}/{ownership}: input/construct/access/cursor/first/resume requested bytes {observations[0]}")
        failed = subprocess.run([binary, "64", "shared", "200000", "0"], capture_output=True, timeout=5)
        if failed.returncode == 0 or b"ROC_ASSERT_FAILED" not in failed.stderr:
            raise RuntimeError(f"{mode}: schedule definition allocation failure control failed")
