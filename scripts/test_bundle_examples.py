...
def copy_internal_examples(destination: Path, core: str, zones: str) -> list[Path]:
    ...
    from update_example_urls import copy_examples
    entries = []
    for name in ("appointment_display", "zoned_appointment", "clock_deadline_checks", "invoice_report", "invoice_report_checks", "schedule_exchange"):
...
def check_output(example: Path, actual: str, *, expected_dir: Path | None = None,
    ...
    if example.parent.name in {"booking_exchange", "archive_search", "staffing", "appointment_display", "zoned_appointment", "clock_deadline_checks", "invoice_report", "invoice_report_checks", "schedule_exchange"} and not expected.is_file():
