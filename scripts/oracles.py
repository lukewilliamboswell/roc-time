...
def refresh_julian() -> None:
    ...
    manifest = {
        "version": 1, "profile": PROFILE.replace("gregorian", "julian"),
...
def replay(name: str) -> None:
    ...
    print(f"PASS {name} oracle: {count} observations in {report['seconds']:.2f}s; evidence {session}")
