...
The test roots `tests/precision/main.roc`, `tests/spans/main.roc`, and `tests/coverage/main.roc`
...
`SpanCase`, or `CoverageCase`). Generators and semantic checks live in those
...
On Apple Silicon macOS, all three targets built from the release URL, replayed
