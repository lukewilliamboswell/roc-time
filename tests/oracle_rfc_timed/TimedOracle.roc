...
observe = |input| {
	parsed = match ICalTimedRule.parse({ start: at(input, 0), rule: at(input, 1), mode: Utc, duration: "PT1S", inclusions: values(at(input, 2)), exclusions: values(at(input, 3)), periods: [] }) {
		Ok(value) => value
...
label = |text| match ICalDateTime.parse(text) {
