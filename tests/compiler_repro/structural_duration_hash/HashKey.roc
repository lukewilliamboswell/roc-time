...
HashKey :: { duration : TimedOccurrence.Duration }.{
	...
	is_eq = |a, b| match (a.duration, b.duration) {
		...
		(Calendar(x), Calendar(y)) => Calendar.Delta.to_components(x.delta) == Calendar.Delta.to_components(y.delta) and x.invalid_date == y.invalid_date and x.tail == y.tail and x.occurrence == y.occurrence and x.gap == y.gap
		_ => Bool.False
	}
...
# The unused Calendar branch is still specialized for the declared key type.
duration_key = |duration| match duration {
	Coordinate(value) => Coordinate(PosixDelta.to_microseconds(value))
	Calendar(value) => Calendar({ components: Calendar.Delta.to_components(value.delta), invalid_date: value.invalid_date, tail: value.tail })
}
