...
# Two entries suffice. Inspect the list before passing it to Json.
main! = |args| {
...
convert = |duration, overrides| {
	...
	for entry in overrides {
		$fields = $fields.append(entry.source)
		match entry.ending {
			After(value) => {
				$fields = $fields.append("after").concat(duration_fields(value))
			}
			AtBoundary(value) => {
				$fields = $fields.append("boundary").append(value.to_str())
			}
			AtLocal(value) => {
				$fields = $fields.append("local").append(value).concat(["unique", "", "reject"])
			}
		}
	}
	$fields
